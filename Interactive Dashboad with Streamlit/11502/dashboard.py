...
if not st.sidebar.checkbox("Hide" , True):
    ...
    if select == 'Histogram':
        fig = px.bar(sentiment_count, x='Sentiment', y='Tweets', color='Sentiment', color_discrete_sequence=['#2E8B57','#1E90FF','#FF6347'], height=500)
        st.plotly_chart(fig)
    else:
...
if len(choice) > 0:
    ...
    fig_choice = px.histogram(choice_data, x='airline', y='airline_sentiment', 
                              ...
                              height=600, width=800, color_discrete_sequence=['#2E8B57','#1E90FF','#FF6347'])
